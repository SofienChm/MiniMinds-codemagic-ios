...
import { StaticFeesService, CreateGroupStaticFeeDto } from '../static-fees.service';
...
@Component({
  selector: 'app-add-static-fee-group',
  standalone: true,
  imports: [CommonModule, FormsModule, ReactiveFormsModule, TitlePage, NgSelectModule, TranslateModule],
  templateUrl: './add-static-fee-group.html',
  styleUrls: ['./add-static-fee-group.scss']
})
export class AddStaticFeeGroupComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.pageTitleService.setTitle(this.translateService.instant('STATIC_FEES_PAGE.ADD_GROUP_FEE'));
    this.initForm();
    ...
    this.http.get<any[]>(`${ApiConfig.ENDPOINTS.CHILDREN}`).subscribe({
      next: (list) => {
        this.children = list.map(c => ({
          ...
          parentId: c.parentId,
          parentName: c.parent ? `${c.parent.firstName} ${c.parent.lastName}` : undefined,
  ...
  createGroupFees(): void {
    ...
    const dto: CreateGroupStaticFeeDto = {
    ...
    this.staticFeesService.createGroupFees(dto).subscribe({
      next: (result) => {
        this.simpleToastService.success(this.translateService.instant('STATIC_FEES_PAGE.GROUP_CREATED', { count: result.count }));
        this.router.navigate(['/static-fees']);
      },
      error: (err) => {
  ...
  onChildrenChange(childIds: number[]): void {
    if (!childIds || childIds.length === 0) {
      return;
    }

    const selected = this.children.filter(c => childIds.includes(c.id));
    const parentIds = new Set(selected.map(c => c.parentId).filter(Boolean));

    // Auto-fill payer info only when a single parent is involved
    if (parentIds.size === 1) {
      const first = selected[0];
      if (first?.parentName) {
        this.staticFeeForm.patchValue({
          payerName: first.parentName,
          payerEmail: first.parentEmail || ''
        });
      }
    }
  }
